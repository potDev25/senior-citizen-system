...
export default function TicketingTable({title, children, dateId, time, getManifestData}) {
    ...
    useEffect(() => {
        axiosClient.get(`/get-passenger-manifest/`)
        .then(({data}) => {
  ...
  return (
    <div className='bg-white shadow-sm h-fit md:w-full sm:w-full sm:scroll-auto px-5 py-2 rounded'>
       ...
       <h1 className='text-md flex items-center mb-4 font-bold tracking-wide uppercase'><FaIcon.FaUserCheck/>&nbsp; Seniors</h1>
       <div className='flex items-center justify-between mb-4'>
            ...
            <div className="relative">
                ...
                <input type="search" id="default-search" onChange={ev => setStringSearch(ev.target.value)} className={(search ? 'w-[90%]' : 'w-[50px]') + " transition-all ease-in-out block py-2 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 ml-2"} required />
            </div>
                        ...
                        manifest ? <>
                            {
                                manifest.filter((data) => {
                                    return stringSearch.toLowerCase() === '' ? data : data.first_name.toLowerCase().includes(stringSearch) || data.last_name.toLowerCase().includes(stringSearch)
                                }).map((data, id) => (
