...
export default function ManualTicketing({handleLoading, show, hideModal, passengers, setPassengerProfile, dateId}) {
            ...
            <TEModalContent>
              ...
              <table className={' bg-gray-100 md:table-auto w-full text-sm'}>
                ...
                <tbody>
                  {
                    passengers.filter((u) => {
                      return search.toLowerCase() === '' ? u : u.name.toLowerCase().includes(search) || u.address.toLowerCase().includes(search)
                    }).map(u => (
