...
export default function WithMinor({handleLoading, show, hideModal, passengerId, dateId}) {
...
const errorlert = (text) => {
  ...
  const handleSubmit = (e) => {
    // console.log(route);
      ...
      axiosClient.put(`/ticket/with_minor/${passengerId}/${dateId}`, minor)
          .then(({data}) => {
              setErrors([]);
              successAlert('Minor Added Successfully')
              setBtnLoading(false)
              handleLoading(true)
              handleHideModal()
          })
            ...
            <TEModalContent>
              ...
              <TEModalBody>
                <div className='grid grid-cols-2 gap-5 p-5'>
                        ...
                        <input
                            onChange={ev => setMinor({...minor, name: ev.target.value})} 
                            type="text" id="first_name" 
                            placeholder='Enter Name'
                            className={` ${errors.name ? 'border-red-500' : 'border-gray-300'} border w-full  text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 `}
                        />
                        {
                            errors.name ? <p className="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">Oh, snapp!</span> {errors.name}</p>
                            : null
                        }
                    </div>
                    <div className='w-full mb-5'>
                        <label htmlFor="" className='text-gray-500'>Age</label>
                        <input
                            onChange={ev => setMinor({...minor, age: ev.target.value})} 
                            type="number" id="first_name" 
                            placeholder='Enter age'
                            className={` ${errors.age ? 'border-red-500' : 'border-gray-300'} border w-full  text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 `}
                        />
                       {
                            errors.age ? <p className="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">Oh, snapp!</span> {errors.age}</p>
                            : null
                        }
                    </div>
                </div>
