...
export default function EditStaff({hideModal, handleLoading, user, provinces}) {
    ...
    function submitUser(ev) {
        ...
        setLoading(true)
        axiosClient.post(`/department/barangay/update/${user.id}`, userInfo, config)
            .then(({data}) => {
                setLoading(false)
                setNotification('Barangay Edited Successfully')
                setErrors([])
                handleLoading()
                hideModal()
            })
        ...
        <div className='bg-white shadow-sm h-fit md:w-[700px] rounded'>
            <div className='flex flex-col items-center justify-center'>
                <div className='h-[200px] w-[200px] rounded-full'>
                    <img src={user && image === '' ?  import.meta.env.VITE_API_BASE_URL + '/storage/' + user.logo : image } 
                ...
                {errors.logo ? <p class="text-red-500 text-xs italic mt-2">{errors.logo}</p> : null}
            </div> 
                ...
                <div className='mt-5 flex items-center justify-center'>
                    ...
                    <div className="flex flex-wrap -mx-3 mb-2">
                        ...
                        <div className="flex flex-wrap -mx-3 mb-6 mt-6">
                            ...
                            <input 
                                onChange={ev => setUserInfo({...userInfo, designation: ev.target.value})}
                                id="grid-last-name" 
                                type="text" 
                                defaultValue={user.designation}
                                placeholder="Doe"
                                class={`${errors.designation ? 'border border-red-500' : 'border-none'} appearance-none block w-full bg-gray-200 text-gray-700  rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white`} 
                                />
                                {errors.designation ? <p class="text-red-500 text-xs italic">{errors.birthdate}</p> : null}
                            </div>
                        </div>
