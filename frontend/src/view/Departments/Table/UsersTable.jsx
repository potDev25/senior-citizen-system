...
import AddStaffModal from '../../Staff/Components/Modals/AddStaffModal';
import AddDepartmentStaffModal from '../Components/Modals/AddDepartmentStaffModal';
...
export default function UsersTable() {
    ...
    useEffect(() => {
        axiosClient.get(`/department/staff/${limit}/${department}?page=${paginate}`)
        .then(({data}) => {
            setPassengers(data.users.data)
  ...
  return (
    <div className='bg-white shadow-sm h-fit md:w-full rounded'>
            <div className='w-full h-10 text-white uppercase p-2 rounded-t-md border-b-2 border-gray-100'>
                <h1 className='uppercase text-md text-gray-500'>Department Staff</h1>
            </div>
            <div className='p-5'>
                    ...
                    <table className='md:table-auto w-full bg-gray-100 text-sm'>
                        <thead className='bg-gray-800 py-5'>
                            <tr>
                                ...
                                <td className='text-sm text-white font-medium px-5 py-2'>POSITION</td>
                        ...
                        <tbody>
                            ...
                            {passengers ? <>
                                {passengers.filter((u) => {
                                    return string.toLowerCase() === '' ? u : u.last_name.toLowerCase().includes(string) || u.first_name.toLowerCase().includes(string)
                                })
                            ...
                            .map(u => (
                                ...
                                <td className='text-md px-5 py-2 font-medium flex gap-5 items-center'>
                                    <div>
                                        <img src={`${import.meta.env.VITE_API_BASE_URL}/storage/${u.photo}`} alt="" className='sm:h-10 w-10 rounded-full'/>
                                    </div>
                                    <div>
                                ...
                                <td className='text-md px-5 py-2x text-gray-500 capitalize'>{u.barangay} {u.city} {u.province}</td>
                                <td className='text-md px-5 py-2x text-gray-500 capitalize'>{u.position}</td>
                                <td className='text-sm px-5 py-2 text-gray-500'>
                                    {
                                        u.block == 1 ? <span className='rounded-[10px] bg-red-300 text-red-500 px-2 text-sm capitalize'>Blocked</span>
                                        : <>
                                            {u.status == '1' && <span className='rounded-[10px] bg-green-300 text-green-500 px-2 text-sm capitalize'>Active</span>}
                                            {u.status == '0' && <span className='rounded-[10px] bg-gray-300 text-gray-500 px-2 text-sm capitalize'>Offline</span>}
                                        </>
                                    }
