...
export default function UpdatePassenger({hideModal, handleLoading, showMediaModal, passenger}) {
    ...
    function submitUser(ev) {
        ev.preventDefault();
        setLoading(true)
        axiosClient.post('/passenger/store', userInfo)
            .then(({data}) => {
                setLoading(false)
                ...
                handleLoading()
                hideModal()
                window.open(`/registration-step/${data.id}`);
            })
            .catch(errors => {
                console.log(errors);
                    ...
                    <form class="w-full" onSubmit={submitUser}>
                        ...
                        <div className="flex flex-wrap -mx-3 mb-6">
                            ...
                            <input 
                                ...
                                type="text" 
                                placeholder="Doe"
                                defaultValue={passenger.contact_number}
                        ...
                        <div className="flex flex-wrap -mx-3 mb-6 mt-6">
                            ...
                            <input 
                                onChange={ev => setUserInfo({...userInfo, age: ev.target.value})}
                                id="grid-last-name" 
                                type="number" 
                                placeholder="Age"
                                class={`${errors.age ? 'border border-red-500' : 'border-none'} appearance-none block w-full bg-gray-200 text-gray-700  rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white`} 
                                />
                                {errors.age ? <p class="text-red-500 text-xs italic">{errors.age}</p> : null}
                            </div>
                        </div>
